---
title: "XMODEL primitive: dff_rst_async_xbit"
document_type: reference
reference_type: primitive
product: XMODEL
primitive: dff_rst_async_xbit
primitive_category: gate
description: "A D flip-flop with xbit-type input/output's and asynchronous reset."
date: 2026-03-11T07:20:43.556915
company: "Scientific Analog, Inc."
license: "Copyright (c) Scientific Analog, Inc."
---

# PRIMITIVE dff_rst_async_xbit
A D flip-flop with xbit-type input/output's and asynchronous reset.

The `dff_rst_async_xbit` primitive models a D flip-flop with a data input `d`, a clock input `clk`, an output `q`, and an asynchronous reset input `rst`. All the input and output signals are xbit-typed. This primitive is suitable when processing digital signals with precise timing information.
...
The parameter `delay_cq` defines the propagation delay from the triggering edge of the clock to the output `q`, and the parameter `delay_rq` defines the propagation delay from the reset input `rst` to the output `q`. These delays are inertial, meaning that if a new input event arrives before the previous change propagates to the output, the output change will be overriden with the one corresponding to the new input event.
...
| delay_rq | real | 0.0 | seconds | reset-to-q delay |
